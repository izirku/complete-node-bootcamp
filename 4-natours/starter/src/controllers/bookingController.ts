/* eslint-disable @typescript-eslint/camelcase */
import { RequestHandler } from 'express'
import Stripe = require('stripe')
import Tour from '../models/tourModel'
import catchAsync from '../utils/catchAsync'
import appError from '../utils/appError'
import {
  deleteOne,
  updateOne,
  createOne,
  retrieveOne,
  retrieveAll
} from './handlerFactory'
import { AppRequest } from '../interfaces'
import Booking from '../models/bookingModel'

const stripe = new Stripe(process.env.STRIPE_SECRET_KEY)

export const getCheckoutSession: RequestHandler = catchAsync(
  async (req: AppRequest, res, _next): Promise<void> => {
    // 1) get the currently booked tour
    const tour = await Tour.findById(req.params.tourId)

    // 2) create checkout session
    const session = await stripe.checkout.sessions.create({
      payment_method_types: ['card'],
      success_url: `${req.protocol}://${req.get('host')}?tour=${
        req.params.tourId
      }&user=${req.user.id}&price=${tour.price}`,
      cancel_url: `${req.protocol}://${req.get('host')}/tour/${tour.slug}`,
      customer_email: req.user.email,
      client_reference_id: req.params.tourId,
      line_items: [
        {
          name: `${tour.name} Tour`,
          description: tour.summary,
          images: ['https://www.natours.dev/img/tours/tour-1-cover.jpg'], // ${tour.imageCover}
          amount: tour.price * 100, // amount in cents
          currency: 'usd',
          quantity: 1
        }
      ]
    })

    // 3) create session as response
    res.status(200).json({
      status: 'success',
      session
    })
  }
)

export const createBookingCheckout: RequestHandler = catchAsync(
  async (req, res, next) => {
    // FIXME: Temporary, insecure
    const { tour, user, price } = req.query
    if (!tour && !user && !price) return next()

    await Booking.create({ tour, user, price })

    // redirect to itself, minus the query params:
    res.redirect(req.originalUrl.split('?')[0])
  }
)

// *****************************************************************************
// C.R.U.D.
export const createBooking = createOne(Booking)
export const getBooking = retrieveOne(Booking)
export const getAllBookings = retrieveAll(Booking)
export const updateBooking = updateOne(Booking)
export const deleteBooking = deleteOne(Booking)

// export const createBooking: RequestHandler = catchAsync(async (req, res, next) => {
//   const booking = await createOne(Booking)
//   res.status(200)
// })
